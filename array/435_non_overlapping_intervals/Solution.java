...
public class Solution {
    ...
    public int eraseOverlapIntervals(Interval[] intervals) {
        ...
        for (int i = 1; i < intervals.length; i++) {
            if(curr.end > intervals[i].start) {
                res++;
                curr = curr.end < intervals[i].end ? curr : intervals[i]; // skip one interval, pick shorter one
            }
            else
                curr = intervals[i]; // if they fit, we pick the last one, as it has bigger end
        }
    ...
    public int eraseOverlapIntervals2222(Interval[] intervals) {
